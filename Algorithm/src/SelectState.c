...
void StabilizeSystem(void)
{
    ...
    if (gAlgorithm.stateTimer == 0) {
        ...
        gAlgorithm.accumulatedAccelVector[X_AXIS] = (real)0.0;
        gAlgorithm.accumulatedAccelVector[Y_AXIS] = (real)0.0;
        gAlgorithm.accumulatedAccelVector[Z_AXIS] = (real)0.0;
    ...
    if (gAlgorithm.stateTimer > 0) {
        ...
        if (gpsUsedInAlgorithm()) {
            ...
            if (gGpsDataPtr->gpsValid) {
                ...
                //   switch is set.
//                BOOL magReadingsPossible = magUsedInAlgorithm();  TODO ???
                int magReadingsPossible = platformHasMag();
                ...
                BOOL canCalcHeading = magReadingsPossible || velGoodForTrans;
...
// ) && gWorldMagModel.validSoln;
                ...
                if ( canCalcHeading &&
                     ...
                     !gAlgorithm.Behavior.bit.freeIntegrate &&
                     ( gMagAlign.state == MAG_ALIGN_STATUS_TERMINATION ||
...
static uint8_t _InitINSFilter(void)
{
    ...
    // the function to here, prevents the Hard Fault Exception seen in stm32f2xx_it.c - WHY?
    real tmp[7][7];
...
static BOOL _AccumulateFieldVectors(void)
{
    ...
    gAlgorithm.accumulatedAccelVector[X_AXIS] += (real)gEKFInputData.accel_B[X_AXIS];
    gAlgorithm.accumulatedAccelVector[Y_AXIS] += (real)gEKFInputData.accel_B[Y_AXIS];
    gAlgorithm.accumulatedAccelVector[Z_AXIS] += (real)gEKFInputData.accel_B[Z_AXIS];
    ...
    {
        gAlgorithm.accumulatedMagVector[X_AXIS] += (real)gEKFInputData.magField_B[X_AXIS];
        gAlgorithm.accumulatedMagVector[Y_AXIS] += (real)gEKFInputData.magField_B[Y_AXIS];
        gAlgorithm.accumulatedMagVector[Z_AXIS] += (real)gEKFInputData.magField_B[Z_AXIS];
    } else {
        gAlgorithm.accumulatedMagVector[X_AXIS] = (real)0.0;
        gAlgorithm.accumulatedMagVector[Y_AXIS] = (real)0.0;
        gAlgorithm.accumulatedMagVector[Z_AXIS] = (real)0.0;
...
static BOOL _AverageFieldVectors(uint16_t pointsToAverage)
{
    ...
    if (platformHasMag())
    {
        ...
        gAlgorithm.averagedMagVector[X_AXIS] = (real)0.0;
        gAlgorithm.averagedMagVector[Y_AXIS] = (real)0.0;
        gAlgorithm.averagedMagVector[Z_AXIS] = (real)0.0;
    }
...
static void _ResetAlgorithm(void)
{
    ...
    for (elemNum = X_AXIS; elemNum <= Z_AXIS; elemNum++) {
        gKalmanFilter.rateBias_B[elemNum]      = (real)0.0;
